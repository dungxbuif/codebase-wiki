...
| --- | --- | --- | --- | --- |
| `curl -fsSL https://raw.githubusercontent.com/dungxbuif/codebase-wiki/master/scripts/install-codewiki-skill.sh \| bash` | install command | GitHub/repo access plus optional Cargo build | implemented | Installs `skill/codewiki` into `$CODEX_HOME/skills/codewiki`, builds `bin/codewiki` when Cargo is available, and keeps copied Rust companion source as fallback. |
